...
def add_department(session: Session):
    """Adds an instance of a department, accepting only valid input = unique constraints"""
    unique_abbrev: bool = False
    ...
    while not unique_abbrev:
        abbreviation = input("Department abbreviation--> ")
        # counts the occurrences of abbreviation in Department table that match abbreviation input
        abbreviation_count = session.query(Department).filter(Department.abbreviation == abbreviation).count()
        # if abbreviation occurrence == 0, unique abbreviation is True, else False
        unique_abbrev = abbreviation_count == 0
        if not unique_abbrev:
...
def add_student_LetterGrade(sess):
    """
    Add a student to a section as a LetterGrade.
    :param sess: The current database connection.
    :return:    None
    """
    student: Student
    ...
    while not unique_student_section:
        ...
        if not unique_student_section:
            print("That section already has that student enrolled in it.  Try again.")
    min_acceptable = input("Letter grade--> ")
    letter_grade = LetterGrade(section, student, min_acceptable)
