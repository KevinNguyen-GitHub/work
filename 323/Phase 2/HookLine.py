...
class HookLine(Base):
    ...
    doors_rooms_num = Column(Integer, ForeignKey('doors.rooms_num'), primary_key=True, nullable=False)
    doors_rooms_buildings_name = Column(String(50), ForeignKey('doors.rooms_buildings_name'), primary_key=True, nullable=False)
    hooks_id = Column(Integer, ForeignKey('hooks.id'), primary_key=True, nullable=False)
